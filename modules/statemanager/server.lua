...
--- @return string?
function SVStateManager:getStateIdByMetadataKey(metadataKey)
  for stateId, state in pairs(self.states) do
    if state.fields.metadata == metadataKey then
      return stateId
    end
  end
end

--- @param stateBagKey string
--- @return string?
function SVStateManager:getStateIdByStateBagKey(stateBagKey)
  for stateId, state in pairs(self.states) do
    if state.fields.stateBag == stateBagKey then
      return stateId
    end
  end
end
...
function SVStateManager:correctStateValue(key, value, src)
  if not key or not src then return end
